...
with st.sidebar:
    ...
    with st.expander("📋 **File Format Guide**", expanded=False):
        ...
        - **Look Up** sheet, columns J:N
        - From last month's automated output
        """)
...
with col_right:
    ...
    prior_working_file = st.file_uploader(
        "📈 Previous Working",
        type=["xlsx"],
        help="Last month's output (Look Up sheet carries forward CIF mappings)",
        key="prior"
    )
...
with col_info:
    if all_files_ready:
        ...
        if not prior_working_file:
            missing.append("Previous Working")
    ...
    with st.spinner("🔄 Processing your data... This typically takes 40-60 seconds"):
        try:
            with st.status("📊 Running automated pipeline...", expanded=True) as status:
                ...
                st.write("📥 **Stage 4/6**: Loading carried-forward lookup master...")
                prior_master = etl.load_prior_lookup_master(prior_working_file)
                st.write(f"   ✓ Loaded {len(prior_master):,} known agreements")
            ...
            if needs_cif > 0:
                st.warning(
                    f"""
                    ⚠️ **{needs_cif:,} NEW AGREEMENTS DETECTED**
                    
                    These customers appear for the first time in this month's DCR. Their CIF/Zone/Sub Region 
                    mapping is blank and needs manual entry. 
                    
                    👉 **Action**: Open the "Look Up Master" tab below, find blank CIF rows, fill them from your LMS, 
                    save the file, and use it as next month's "Previous Working" input.
                    """,
                    icon="⚠️"
                )
...
if "workbook_buffer" in st.session_state:
    ...
    with tab6:
        st.markdown("#### Cumulative Agreement Master")
        st.info("✓ Carries forward from previous month + refreshes with this month's data")
    ...
    with tab7:
        ...
        if col_select:
            st.dataframe(df[col_select].head(100), use_container_width=True, use_container_width=True)
        if len(df) > 100:
