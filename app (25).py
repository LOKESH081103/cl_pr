...
DAY_PROCESS_SOURCE_ZONE = "PAN INDIA"
DAY_PROCESS_BUCKET_ZONECOL = {"FRESH": "fresh", "1-29": "1-29", "30-59": "30-59", "60-89": "60-89"}
...
def update_day_process_history(history, zone_data, selected_time):
    """Returns a new history dict with the selected time-slot filled in using the
    PAN INDIA row of the Sub-Zone Cut table (not the top boxes), leaving every
    other slot untouched."""
    updated = {b: dict(v) for b, v in history.items()}
    for disp_name, zone_col in DAY_PROCESS_BUCKET_ZONECOL.items():
        v = get_raw_val(zone_data, DAY_PROCESS_SOURCE_ZONE, zone_col)
        updated[disp_name][selected_time] = v if v is not None else 0
    return updated
...
def build_html(bucket_data, zone_data, timestamp_str, history):
    # Map each box key to its PAN INDIA zone_data column (None = no zone column -> fall back to bucket_data)
    BOX_ZONE_COL = {
        'Fresh':             'fresh',
        '1-29':              '1-29',
        '1-29 NORM':         None,
    ...
    for key, label, bg, tc in BOX_CONFIGS:
        ...
        fl  = bd.get('flow', 0)
        zone_col = BOX_ZONE_COL.get(key)
        if zone_col is not None:
            eff = get_raw_val(zone_data, 'PAN INDIA', zone_col) or 0
        else:
            eff = bd.get('eff', 0)
        boxes_html += f"""
        <div class="box">
...
start_clicked = st.button("▶️ Start Process", disabled=(uploaded_file is None))
...
current_sig = (
    ...
    prev_file.name if prev_file else None,
    prev_file.size if prev_file else None,
    selected_time,
)

# Any change to the inputs invalidates a previous run — require a fresh click.
if current_sig != st.session_state.processed_sig:
    st.session_state.processed = False
...
if not uploaded_file:
    ...
    st.info("File ready. Click **▶️ Start Process** above to generate the dashboard.")
else:
    try:
    ...
    history = update_day_process_history(history, zone_data, selected_time)
