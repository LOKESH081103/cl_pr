...
def extract_dashboard_metrics(df, zone_name="PAN_INDIA"):
    """
    Extract zone-specific metrics from the main dashboard.
    Returns empty - the actual metrics come from extract_all() and extra_t/extra_y.
    """
...
def extract_s2_metrics(dfs_dict, zone_name=None):
    ...
    Zone name is in column 2 (index 2).
    Column E (index 4) = S2 Projection
    Column G (index 6) = S2 RB (Actual)
    Column I (index 8) = S2 LMTD
    
    PAN INDIA is at row 5 (index 4).
    Other zones are in subsequent rows with same column indices.
    """
    ...
    if dfs_dict:
        try:
            ...
            if zone_name and zone_name != 'PAN_INDIA':
                # Search for the zone in column 2
                target = zone_name.upper().replace('_', ' ')
                found_idx = None
                for idx, row in df.iterrows():
                    if len(row) > 2:
                        cell = str(row.iloc[2]).strip().upper()
                        if cell == target or cell == target.replace(' ', '_'):
                            found_idx = idx
                            break
                if found_idx is None:
                    # Zone not found, use PAN INDIA row
                    found_idx = 4
            else:
                # Use PAN INDIA row (index 4)
                found_idx = 4
            
            if len(df) > found_idx:
                row = df.iloc[found_idx].tolist()
                metrics['s2_proj'] = num(safe_get(row, 4))   # Column E
                metrics['s2_rb']   = num(safe_get(row, 6))   # Column G
                metrics['s2_lmtd'] = num(safe_get(row, 8))   # Column I
        except Exception as e:
...
def extract_s3_metrics(dfs_dict, zone_name=None):
    ...
    Zone name is in column 0 (index 0).
    Metrics stay in the same columns as you go down rows.
    
    PAN INDIA is at row 4 (index 4).
    """
    metrics = {'s3_concern_flow': None, 's3_concern_lmtd': None, 's3_concern_proj': None}
    if dfs_dict:
        try:
            # Find the S3 Concern summary sheet
            ...
            if df_sum is not None and not df_sum.empty:
                # Determine which row to read
                if zone_name and zone_name != 'PAN_INDIA':
                    # Search for the zone row in column 0
                    target = zone_name.upper().replace('_', ' ')
                    found_idx = None
            ...
            # Extract projection from Top Sheet - zone-aware
            ...
            if df_top is not None and not df_top.empty:
                # Search for the zone row in Top Sheet column 0
                if zone_name and zone_name != 'PAN_INDIA':
                    target = zone_name.upper().replace('_', ' ')
                    found_proj_idx = None
                    for idx, row in df_top.iterrows():
                        if len(row) > 0:
                            ...
                            if cell == target or cell == target.replace(' ', '_'):
                                found_proj_idx = idx
                                break
                    if found_proj_idx is None:
                        # Zone not found, use PAN INDIA row (13)
                        found_proj_idx = 13
                else:
                    # Use PAN INDIA row (index 13)
                    found_proj_idx = 13
                
                if len(df_top) > found_proj_idx:
                    row_top = df_top.iloc[found_proj_idx].tolist()
...
def find_data_rows(df):
    ...
    for _, row in df.iterrows():
        cell = str(row.iloc[1]).strip().upper().replace(' ', '_')  # Zone name is in column 1
        if cell in targets:
...
def get_row(rows, name):
    ...
    for r in rows:
        cell = str(r[1]).strip().upper().replace(' ', '_')  # Zone name is in column 1
        if cell == norm:
            return r
    return None
