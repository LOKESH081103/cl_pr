...
ZONE_CONFIG = {
    ...
    'MP_AND_GUJARAT': {
        'label': 'MP & Gujarat',
        'subzones': ['MADHYA_PRADESH','GUJARAT'],
        'display': {'MADHYA_PRADESH':'Madhya Pradesh','GUJARAT':'Gujarat'},
    },
    'ROMH_AND_MUMBAI': {
        'label': 'ROMH & Mumbai',
        'subzones': ['ROMH','MUMBAI_&_GOA'],
        'display': {'ROMH':'ROMH','MUMBAI_&_GOA':'Mumbai & Goa'},
    },
}
...
def bottom2_flow(all_metrics, flow_key, subzones, display):
    ...
    return sorted(vals, key=lambda x: x[1], reverse=True)[:2]
...
def create_report_image(today_m, yest_m, extra_t, extra_y, report_date, view_key):
    ...
    try:
        font_title    = ImageFont.truetype("arial.ttf", 112)
        font_subtitle = ImageFont.truetype("arial.ttf", 48)
        font_section  = ImageFont.truetype("arial.ttf", 56)
        ...
        with st.spinner("Parsing files and building report…"):
            try:
                ...
                if not today_rows or not yest_rows:
                    ...
                    # Only override with S3 RB file values if they are not None
                    _s3rb_t = {k: v for k, v in s3rb_t_extra.items() if v is not None}
                    _s3rb_y = {k: v for k, v in s3rb_y_extra.items() if v is not None}
                    extra_t = {**dash_t_extra, **s2_t_extra, **s3_t_extra, **_s3rb_t}
                    extra_y = {**dash_y_extra, **s2_y_extra, **s3_y_extra, **_s3rb_y}
