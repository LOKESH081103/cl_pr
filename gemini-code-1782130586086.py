...
# ── Section 2 row and column indices (0-based) ────────────────────────────────
ROW_TARGET     = 16  # 17th row in Excel
COL_R          = 17  # Column R
COL_Z          = 25  # Column Z
COL_BA         = 52  # Column BA
COL_BH         = 59  # Column BH
...
def create_report_image(today_m, yest_m, extra_t, extra_y, report_date):
    ...
    rows_s2 = [
        ("1-29 Norm", fmt_num(extra_t.get('norm_129')), fmt_num(extra_y.get('norm_129')), fmt_num(extra_t.get('norm_129_lmtd')), "---"),
        ("Stage 2 Roll Back", fmt_num(extra_t.get('s2_rb')), fmt_num(extra_y.get('s2_rb')), "---", "---"),
        ("Stage 3 Concern Flow", "---", "---", "---", "---"),
...
def extract_section2_metrics(df):
    metrics = {
        'norm_129': None,
        'norm_129_lmtd': None,
        's2_rb': None,
    ...
    if df.empty or len(df) <= ROW_TARGET:
        return metrics
    try:
        row = df.iloc[ROW_TARGET]
        metrics['norm_129'] = num(row.iloc[COL_R]) if len(row) > COL_R else None
        metrics['norm_129_lmtd'] = num(row.iloc[COL_Z]) if len(row) > COL_Z else None
        metrics['s2_rb'] = num(row.iloc[COL_BA]) if len(row) > COL_BA else None
        metrics['s3_rb'] = num(row.iloc[COL_BH]) if len(row) > COL_BH else None
    except Exception as e:
        st.warning(f"Error extracting row 17 metrics: {e}")
    return metrics
...
def render_report(today_m, yest_m, extra_t, extra_y, date_str):
    ...
    rows2 = [
        {"Bucket": "1-29 Norm", "Today": fmt_num(extra_t.get('norm_129')), "Yesterday": fmt_num(extra_y.get('norm_129')), "LMTD": fmt_num(extra_t.get('norm_129_lmtd')), "Projection Crs.": "---"},
        {"Bucket": "Stage 2 Roll Back", "Today": fmt_num(extra_t.get('s2_rb')), "Yesterday": fmt_num(extra_y.get('s2_rb')), "LMTD": "---", "Projection Crs.": "---"},
        {"Bucket": "Stage 3 Concern Flow", "Today": "---", "Yesterday": "---", "LMTD": "---", "Projection Crs.": "---"},
        {"Bucket": "Stage 3 Roll Back", "Today": fmt_num(extra_t.get('s3_rb')), "Yesterday": fmt_num(extra_y.get('s3_rb')), "LMTD": "---", "Projection Crs.": "---"},
    ]
        ...
        with st.spinner("Parsing dashboards…"):
            try:
                ...
                if not today_rows or not yest_rows:
                    ...
                    # Extract Section 2 specific data from row 17 directly
                    extra_t = extract_section2_metrics(today_df_main)
                    extra_y = extract_section2_metrics(yest_df_main)
