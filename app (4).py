...
def extract_dashboard_metrics(df, zone_name="PAN_INDIA"):
    ...
    Searches for the specific zone row instead of always using row 16 (PAN INDIA).
    """
    ...
    try:
        # Search for the zone row
        found_idx = None
        for idx, row in df.iterrows():
            zone = str(row.iloc[1]).strip().upper().replace(" ", "_")
            if zone == zone_name.upper().replace(" ", "_"):
                found_idx = idx
                break
        
        # If zone not found, use PAN INDIA (row 16)
...
def extract_s2_metrics(dfs_dict, zone_name=None):
    """
    Extract Stage 2 metrics. If zone_name is provided, find that specific zone row.
    Otherwise default to PAN INDIA (row 4).
    """
...
def extract_s3_metrics(dfs_dict, zone_name=None):
    """
    Extract Stage 3 metrics. If zone_name is provided, find that specific zone row.
    Otherwise default to PAN INDIA (row 4).
    """
    ...
    if dfs_dict:
        try:
            ...
            if df_top is not None and not df_top.empty:
                # Search for the zone row in Top Sheet
                if zone_name and zone_name != 'PAN_INDIA':
                    target = zone_name.upper().replace('_', ' ')
                    found_proj_idx = None
