...
def create_report_image(today_m, yest_m, extra_t, extra_y, report_date):
    ...
    # ── Image dimensions ──
    width = 1600
    height = 2800 # Increased height to accommodate the new section
...
def extract_extra_metrics(dfs):
    metrics = {}
    new_summary = dfs.get("NEW SUMMARY")
    
    if new_summary is not None and not new_summary.empty:
        try:
            # Stage 2 Roll Back: 5th Row (index 4), Column G (index 6)
            metrics['s2_rb_today'] = num(new_summary.iloc[4, 6])
        except Exception:
            metrics['s2_rb_today'] = None
    else:
        metrics['s2_rb_today'] = None
        ...
        with st.spinner("Parsing dashboards…"):
            try:
                ...
                if not today_rows or not yest_rows:
                    ...
                    extra_t = extract_extra_metrics(today_dfs)
                    extra_y = extract_extra_metrics(yest_dfs)
